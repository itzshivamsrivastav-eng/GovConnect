...
export default function SchemeCard({ scheme, match }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 hover:shadow-sm transition-all flex flex-col">
      ...
      <h4 className="font-heading font-semibold text-navy-900 mb-1">{scheme.name}</h4>
      <p className="text-xs text-navy-600 mb-2">{scheme.category}</p>
      ...
      {match && match.likelyEligible && (
        <p className="text-xs text-gray-500 mb-3 flex items-start gap-1">
          <Award size={14} className="text-navy-600 mt-0.5 shrink-0" />
          {match.reason}
        </p>
      )}
      <Link
