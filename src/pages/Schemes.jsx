...
export default function Schemes() {
  ...
  const categories = useMemo(() => ['All', ...new Set(ranked.map((r) => r.scheme.category))], [ranked]);
  ...
  return (
    <DashboardLayout>
      <h1 className="font-heading text-2xl font-bold text-navy-900 mb-1">Government Schemes</h1>
      <p className="text-sm text-gray-500 mb-2">
        Prototype/fictional schemes used to demonstrate rule-based eligibility matching, ranked by match percentage.
      </p>
      <p className="text-xs text-gray-400 mb-6">
        Eligibility shown by GovConnect is an estimate based on the information provided. Final eligibility is
      ...
      {filtered.length === 0 ? (
        <EmptyState title="No schemes found" description="Try a different search term or category." />
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(({ scheme, match }) => (
            <SchemeCard key={scheme.id} scheme={scheme} match={match} />
          ))}
        </div>
      )}
    </DashboardLayout>
